...
const tracker = {
	...
	queryLastLocationsByIds: 'select simulation.* from simulation_data_1 as simulations right join (select max(unix_timestamp(time)) as time from simulation_data_1 group by customer_id where customer_id in (?)) as max on max.time = simulations.time and max.customer_id = simulations.customer_id'
};

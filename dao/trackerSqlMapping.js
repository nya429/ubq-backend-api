...
const tracker = {
	...
	queryLastLocationsByIds: 'select unix_timestamp(simulations.time) as time, simulations.customer_id, simulations.loc_x, simulations.loc_y, simulations.product_id from simulation_data_1 as simulations right join (select customer_id, max(time) as time from simulation_data_1 group by customer_id having customer_id in (?)) as max on max.time = simulations.time and max.customer_id = simulations.customer_id'
};
